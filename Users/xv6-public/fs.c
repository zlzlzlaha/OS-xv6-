...
namex(char *path, int nameiparent, char *name)
{
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    //check directory execution authority
    if(check_mode(ip,myproc()->username,ip->type,3) < 0){
      iunlockput(ip);
      return 0;
    } 
...
int userdel(char * username)
{
   ...
   //can't delete root id and only root user can delete users 
...
//if there is no userlist file , initalize usertable. and make new file.
//if thies is alreay user list file, load them
...
int chmod(char *pathname, int mode)
{
    ...
    // when have root or owner authority  
    if( (check_same(myproc()->username,"root") == 0)  || (check_same(ip->username,myproc()->username) == 0 )){
       ip->mode = new_mode;
       iupdate(ip);
       iunlock(ip);
       end_op();
...
//check_file authority for one of r , w, x 
// if process does not have certain mode for file , return -1
int check_mode(struct inode * ip, char* username, int type,  int rwx)
{
    uint mode;
    ...
    switch(rwx){
       // 1 for checking Read 
      case 1:
          //when process's username is root or owner 
        if(check_same(username,"root") == 0 || check_same(username,ip->username) ==0){
            mode = ip->mode & MODE_RUSR;
            if(mode)
                return 0;
            return -1;
        }
      ...
      //2 for checking Write  
      case 2:
     
        if(check_same(username,"root") == 0 || check_same(username,ip->username) ==0){
            mode = ip->mode & MODE_WUSR;
            if(mode)
                return 0;
            return -1;
        }
      ...
      //3 for chekcing Execute
      case 3:

        if(check_same(username,"root") == 0 || check_same(username,ip->username) ==0){
            mode = ip->mode & MODE_XUSR;
            if(mode)
                return 0;
            return -1;
        }
        else{
         mode = ip->mode & MODE_XOTH;
         if(mode)
             return 0;
         return -1;
        }
